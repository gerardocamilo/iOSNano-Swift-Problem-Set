...
let maxNumOfBugsAllowed = 9
let minimumNumberOfLevels: Int = 6
...
func tooManyBugs(){
    print("The game \"\(game)\" needs to have less than \(maxNumOfBugsAllowed + 1) bugs to be released.")
}

func noMusic(){
    print("The game \"\(game)\" needs to have music to be released.")
}

func needMoreLevels(){
    print("The game \"\(game)\" needs to have at least \(minimumNumberOfLevels) levels to be released.")
}

func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    if (bugs <= maxNumOfBugsAllowed) && music && (levels >= minimumNumberOfLevels){
        release()
    } else {
        if bugs > maxNumOfBugsAllowed {
            tooManyBugs()
        } else if !music {
            noMusic()
        } else if levels < minimumNumberOfLevels {
            needMoreLevels()
        }
    }
}
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    if bike && run {
        ...
        print("\(name) cannot finish Biking nor Running portions of the duathlon.")
    } else if !bike {
        print("\(name) cannot finish Biking portion.")
    } else if !run {
        print("\(name) cannot finish Running portion.")
    }
}
